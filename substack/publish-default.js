...
(async () => {
  ...
  function mdToHtml(md) {
    ...
    for (let line of lines) {
      ...
      if (line.startsWith('## ')) {
        if (inList) { html += '</ul>\n'; inList = false; }
        flushPara();
        html += `<h2>${line.substring(3).replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>')}</h2>\n`;
        continue;
      }
      if (line.startsWith('- ')) {
        flushPara();
        if (!inList) { html += '<ul>\n'; inList = true; }
        let item = line.substring(2).replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>').replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
        html += `<li>${item}</li>\n`;
        continue;
      }
      if (line.match(/^\d+\.\s/)) {
        flushPara();
        if (!inList) { html += '<ol>\n'; inList = true; }
        let item = line.replace(/^\d+\.\s/, '').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>');
        html += `<li>${item}</li>\n`;
        continue;
      }
      if (inList) { html += '</ul>\n'; inList = false; }
  ...
  // Use the Default Chrome profile directly
  const profileDir = '/Users/openclaw/Library/Application Support/Google/Chrome';
  console.log('Launching with Default profile...');
  ...
  if (currentUrl.includes('sign-in') || currentUrl.includes('login')) {
    console.log('ERROR: Not logged in with Default profile.');
    await browser.close();
    process.exit(1);
  }
  ...
  if (!titleEl) {
    ...
    await page.screenshot({ path: path.join(__dirname, 'editor-debug-default.png') });
    await browser.close();
    process.exit(1);
  }
