...
TODO = ROOT / '.prompt-hub/todo/todo-20260429-180603-scan-list.md'
SCRIPT = ROOT / '.prompt-hub/todo/scan_list_20260429_180603_runner.py'
...
def mark_todo_done(processed: int, errors: int) -> None:
    if TODO.exists():
        txt = TODO.read_text()
        txt = txt.replace('- [ ] Inspect LIST.md and process each queued URL top-to-bottom', '- [x] Inspect LIST.md and process each queued URL top-to-bottom')
        txt = txt.replace('- [ ] Create and verify batch recap if URLs were processed', '- [x] Create and verify batch recap if URLs were processed')
        txt = txt.replace('- [ ] Update prompt-hub memory/version/releases and push all changes', '- [x] Update prompt-hub memory/version/releases and push all changes')
