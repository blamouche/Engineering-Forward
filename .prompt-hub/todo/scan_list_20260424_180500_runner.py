...
TODO = ROOT / '.prompt-hub/todo/todo-20260424-180500-scan-list.md'
SCRIPT = ROOT / '.prompt-hub/todo/scan_list_20260424_180500_runner.py'
...
def mark_todo_done() -> None:
    if TODO.exists():
        txt = TODO.read_text()
        txt = txt.replace('- [ ] Inspect `LIST.md` and note batch timestamp', '- [x] Inspect `LIST.md` and note batch timestamp')
        txt = txt.replace('- [ ] Process each queued URL sequentially', '- [x] Process each queued URL sequentially')
        txt = txt.replace('- [ ] Create and verify batch recap', '- [x] Create and verify batch recap')
        txt = txt.replace('- [ ] Update prompt-hub tracking and push remaining changes', '- [x] Update prompt-hub tracking and push remaining changes')
        if '## Review\n- Pending.' in txt:
