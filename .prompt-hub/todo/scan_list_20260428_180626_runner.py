...
TODO = ROOT / '.prompt-hub/todo/todo-20260428-180626-scan-list.md'
SCRIPT = ROOT / '.prompt-hub/todo/scan_list_20260428_180626_runner.py'
...
def extract_main_text(soup: BeautifulSoup) -> str:
    for bad in soup(['script', 'style', 'noscript', 'header', 'footer', 'nav', 'form', 'aside', 'svg']):
        bad.decompose()
    candidates = []
    for selector in ['article', 'main', '[role="main"]', '.post-content', '.article-content', '.entry-content', '.content', '.prose']:
...
def mark_todo_done(processed: int, errors: int) -> None:
    if TODO.exists():
        txt = TODO.read_text()
        txt = txt.replace('- [ ] Process each URL from LIST.md sequentially with article synthesis', '- [x] Process each URL from LIST.md sequentially with article synthesis')
        txt = txt.replace('- [ ] Create batch recap and verify coverage', '- [x] Create batch recap and verify coverage')
        txt = txt.replace('- [ ] Update prompt-hub tracking, version, and releases', '- [x] Update prompt-hub tracking, version, and releases')
        txt = txt.replace('- [ ] Push all remaining commits', '- [x] Push all remaining commits')
