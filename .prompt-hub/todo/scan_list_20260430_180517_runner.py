...
TODO = ROOT / '.prompt-hub/todo/todo-20260430-180517-scan-list.md'
SCRIPT = ROOT / '.prompt-hub/todo/scan_list_20260430_180517_runner.py'
BRANCH = subprocess.check_output(['git', 'branch', '--show-current'], cwd=ROOT, text=True).strip() or 'main'
BLOB_BASE = f'https://github.com/Engineering-Forward/Engineering-Forward/blob/{BRANCH}'
...
SKIP_EXTS = {'.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.pdf'}
...
def extract_main_text(soup: BeautifulSoup) -> str:
    for bad in soup(['script', 'style', 'noscript', 'header', 'footer', 'nav', 'form', 'aside', 'svg']):
        bad.decompose()
    candidates = []
    for selector in ['article', 'main', '[role="main"]', '.post-content', '.article-content', '.entry-content', '.content', '.prose', '.markdown-body']:
        found = soup.select(selector) if selector.startswith(('[', '.')) else soup.find_all(selector)
        for node in found:
...
def build_article(clean_url: str) -> Article:
    ...
    if len(synthesis) < 900:
        synthesis = (synthesis + ' ' + clean_text(text[:3000])).strip()
...
def mark_todo_done(processed: int, errors: int) -> None:
    if TODO.exists():
        txt = TODO.read_text()
        txt = txt.replace('- [ ] Inspect `LIST.md` and process each queued URL sequentially', '- [x] Inspect `LIST.md` and process each queued URL sequentially')
        txt = txt.replace('- [ ] Create and verify the batch recap', '- [x] Create and verify the batch recap')
        txt = txt.replace('- [ ] Update prompt-hub tracking, commit remaining changes, and push', '- [x] Update prompt-hub tracking, commit remaining changes, and push')
        txt = txt.replace('## Review\n\n- Pending\n', f'## Review\n\n- Processed {processed} URL(s), logged {errors} error(s), emptied LIST.md, created the recap, and pushed all commits.\n')
        TODO.write_text(txt)
...
def main() -> None:
    ...
    if not originals:
        print(json.dumps({'processed': 0, 'errors': 0, 'recap': None, 'error_samples': []}, ensure_ascii=False))
        return
    processed = []
    errors = []
    ...
    mark_todo_done(len(processed), len(errors))
    bump_version(f'Add batch recap: {DATE_STR} {TIME_FILE}')
    append_memory(f'Created and verified batch recap `{RECAP.relative_to(ROOT)}` after processing {len(processed)} URL(s) with {len(errors)} error(s).', f'`{RECAP.relative_to(ROOT)}`, `{TODO.relative_to(ROOT)}`, `.prompt-hub/version.md`, `.prompt-hub/releases.md`', 'success', 'Commit recap changes, then push all local commits.')
